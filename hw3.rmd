...
# Bad visualization of the chloropleth map as an unscaled bar graph

west_coast_states <- query("
  SELECT Cand_State AS State, COUNT(*) AS Count
  FROM df
  WHERE Cand_State IN ('CA', 'OR', 'WA', 'NV', 'AK', 'HI', 'AZ', 'UT', 'ID')
  GROUP BY Cand_State
")

ggplot(data = west_coast_states) + geom_col(mapping = aes(x = Cand_State, y = Count, fill = State)) +
  labs(x = "States",
       y = "Number of Candidates",
       title = "Canidate Population by West Coast States",
       subtitle = "All political offices taken into consideration.") +
  theme(
...
ggsave("plots_hw3/bad_bar_graph_states.png", width = 8, height = 6, dpi = 300)
```
